...
The packaged root ECM path is now external-schema-only.

Expected CSV inputs beside `ecm_step.py`:
- `params.csv` with lookup-table columns:
  - `Q_Ah`
  - `T_degC`
  - `R0_Ohm`
  - `R_Ohm_1`
...
The packaged `params.csv` and `ecm_step.py` are only a placeholder pair for
that external-style contract. The old SOC-based parameter table is no longer
