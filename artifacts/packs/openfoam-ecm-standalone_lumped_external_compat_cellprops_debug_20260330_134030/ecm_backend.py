...
CELLPROPS_CAPACITY_COLUMNS = ["capacity_Ah", "Qnom_Ah"]
CELLPROPS_TREF_COLUMNS = ["T_ref_degC"]
...
def _validate_supported_inputs(
    ...
    has_capacity = any(name in cellprops_columns for name in CELLPROPS_CAPACITY_COLUMNS)
    if not has_capacity:
        raise ECMBackendError(
            "cellprops.csv is missing required columns for compatibility mode. "
            f"expected_capacity_columns={CELLPROPS_CAPACITY_COLUMNS}; "
            f"optional_temperature_columns={CELLPROPS_TREF_COLUMNS}; "
            f"actual_columns={cellprops_columns}; "
            f"missing_capacity_columns={CELLPROPS_CAPACITY_COLUMNS}"
        )
...
def _adapt_soc_table_to_external_lookup(params_df, cellprops_df, *, temp_anchor_degc: float | None = None):
    import pandas as pd
    ...
    try:
        ...
        raise ECMBackendError(f"Failed to read cell capacity from cellprops.csv: {exc}") from exc

    t_ref_degc = temp_anchor_degc if temp_anchor_degc is not None else _tref_from_cellprops(cellprops_df)

    if capacity_ah is None or not math.isfinite(capacity_ah) or capacity_ah <= 0.0:
        raise ECMBackendError(f"Invalid capacity_Ah in cellprops.csv: {capacity_ah}")
    if t_ref_degc is None or not math.isfinite(t_ref_degc):
        raise ECMBackendError(f"Invalid temperature anchor for adapted lookup table: {t_ref_degc}")
...
def _capacity_from_cellprops(cellprops_df) -> float | None:
    for name in CELLPROPS_CAPACITY_COLUMNS:
        try:
            value = float(cellprops_df[name].iloc[0])
        except Exception:
            continue
...
def _tref_from_cellprops(cellprops_df) -> float | None:
    for name in CELLPROPS_TREF_COLUMNS:
        try:
            value = float(cellprops_df[name].iloc[0])
        except Exception:
            continue
...
def _try_add_derived_column(params_df, target: str):
    ...
    if target in df.columns:
        return df, False
    if target == "tau1_s" and _has_columns(list(df.columns), ["R_Ohm_1", "C_F_1"]):
        df[target] = df["R_Ohm_1"] * df["C_F_1"]
        return df, True
    if target == "tau2_s" and _has_columns(list(df.columns), ["R_Ohm_2", "C_F_2"]):
...
def _collect_missing_params_columns(build_step_cache_fn, params_df):
    import pandas as pd

    probe_df = params_df.copy()
    probe_df.attrs["_ecm_probe_missing_columns"] = True
    missing: list[str] = []
    ...
    def patched_getitem(self, key):
        is_probe = bool(getattr(self, "attrs", {}).get("_ecm_probe_missing_columns"))
        if is_probe:
            if isinstance(key, list):
                missing_names = [name for name in key if name not in self.columns]
                if missing_names:
                    df = self.copy()
                    df.attrs["_ecm_probe_missing_columns"] = True
                    for name in missing_names:
                        _record(str(name))
                        df[str(name)] = float("nan")
                    return original_getitem(df, key)
            elif isinstance(key, str) and key not in self.columns:
                _record(key)
                return pd.Series([float("nan")] * len(self.index), index=self.index, name=key)
...
def _synthesize_missing_params_column(params_df, cellprops_df, missing_name: str):
    ...
    if missing_name == "tau1_s":
        return _try_add_derived_column(params_df, missing_name)
    if missing_name == "tau2_s":
        return _try_add_derived_column(params_df, missing_name)
    if missing_name == "DOD" and "SOC" in params_df.columns:
...
class EcmStepBackend(BaseECMBackend):
    ...
    def _build_runtime_lookup_from_soc(self, temp_anchor_degc: float):
        params_df = _adapt_soc_table_to_external_lookup(
            self._soc_params_source_df,
            self._cellprops_df,
            temp_anchor_degc=float(temp_anchor_degc),
        )
        params_df = _add_external_alias_columns(params_df, self._cellprops_df)
        lookup_cache = self._build_step_cache_fn(params_df)
        self._params_df = params_df
        self._lookup_cache = lookup_cache
        self._needs_runtime_temp_anchor = False
    ...
    def __init__(
        ...
        *,
        ...
        self._build_step_cache_fn = module.build_step_cache
        self._needs_runtime_temp_anchor = False
        self._soc_params_source_df = None
        if not _has_columns(params_cols, EXTERNAL_PARAMS_COLUMNS):
            temp_anchor_degc = _tref_from_cellprops(cellprops_df)
            if temp_anchor_degc is None:
                temp_anchor_degc = 0.0
                self._needs_runtime_temp_anchor = True
            self._soc_params_source_df = params_df.copy()
            params_df = _adapt_soc_table_to_external_lookup(
                params_df,
                cellprops_df,
                temp_anchor_degc=temp_anchor_degc,
            )
        ...
        preflight_missing, preflight_error = _collect_missing_params_columns(
            module.build_step_cache,
            params_df,
        )
        unresolved_preflight = [name for name in preflight_missing if name not in params_cols]
        if unresolved_preflight:
            raise ECMBackendError(
                "build_step_cache preflight found unresolved params.csv requirements: "
                f"preflight_missing_columns={unresolved_preflight}; "
                f"module={module_path}; params_path={Path(params_path).resolve()}; "
                ...
                f"preflight_error={preflight_error}"
            )
        ...
        self._lookup_cache = lookup_cache if not self._needs_runtime_temp_anchor else None
    ...
    def step(
        ...
        *,
        ...
        if self._lookup_cache is None:
            self._build_runtime_lookup_from_soc(float(t_cell_degc))
