    ...
    restartCheckpointRestored_(false)
{
    ...
    const bool restoredCheckpoint = restoreRestartCheckpoint();
    const bool loadedLastGood = (!restoredCheckpoint && readLastGoodSnapshot());

    if (restoredCheckpoint && (Pstream::master() || !Pstream::parRun()))
    {
