...
IN_ROOT = ROOT / "artifacts" / "validation" / "lumped_forward"
...
def _latest_json(test: str) -> Path:
    ...
    if not files:
        raise RuntimeError(f"No metrics JSON found for {test}")
    return files[-1]
...
def _plot_timestep(metrics: dict) -> tuple[Path, Path]:
    ...
    p1 = PLOT_ROOT / "lumped_forward_timestep_tcap.png"
    p2 = PLOT_ROOT / "lumped_forward_timestep_qsum.png"
    ...
    plt.title("ECM Timestep Check: Final Temperature vs deltaT")
    ...
    plt.title("ECM Timestep Check: Final Heat vs deltaT")
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(p2)
    plt.close()
...
def _plot_energy(metrics: dict) -> Path:
    p = PLOT_ROOT / "lumped_forward_energy_balance.png"
    ...
    x = np.arange(2)
    vals = np.asarray([expected, stored])
    plt.bar(x, vals, width=0.55, color=["#4c78a8", "#f58518"])
    plt.xticks(x, ["Expected", "Stored"])
    plt.ylabel("Energy [J]")
    plt.title(f"Adiabatic Energy Balance (error = {err:.3f}%)")
...
def _plot_mesh(metrics: dict) -> tuple[Path, Path]:
    ...
    p1 = PLOT_ROOT / "lumped_forward_mesh_tmax.png"
    p2 = PLOT_ROOT / "lumped_forward_mesh_tcap.png"

    plt.figure(figsize=(7.2, 3.8), dpi=170)
    plt.plot(cells, tmax, marker="o", lw=1.8)
    plt.xlabel("jellyRoll cells [-]")
    plt.ylabel("Final Tmax(jellyRoll) [K]")
    plt.title("Mesh Check: Peak Temperature vs Mesh Size")
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(p1)
    plt.close()
    ...
    plt.xlabel("jellyRoll cells [-]")
    plt.ylabel("Final capacity-weighted T [K]")
    plt.title("Mesh Check: Capacity-Weighted T vs Mesh Size")
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(p2)
    plt.close()
...
def _case_table(rows: list[tuple[str, str]]) -> Table:
    ...
    tbl = Table(data, colWidths=[180, 320])
    tbl.setStyle(
        TableStyle(
            [
...
def main() -> int:
    ...
    p_zero = _plot_ecm_series(ecm_zero, "lumped_forward_ecm_zero_qsum.png", "ECM Zero-Current: Q_sum_check vs Time")
    p_cur = _plot_ecm_series(ecm_current, "lumped_forward_ecm_current_qsum.png", "ECM Fixed-Current: Q_sum_check vs Time")

    stamp = dt.datetime.now(dt.UTC).strftime("%Y%m%d_%H%M%S")
    pdf = OUT_ROOT / f"lumped_forward_validation_report_{stamp}.pdf"

    styles = getSampleStyleSheet()
    doc = SimpleDocTemplate(str(pdf), pagesize=A4, title="Lumped Forward Validation Report")
    story = [
        Paragraph("Lumped CFD-ECM Forward Validation Report", styles["Title"]),
        Spacer(1, 8),
        Paragraph("Plot-first report. Tables are limited to case metadata and run settings.", styles["Normal"]),
        Spacer(1, 10),
    ]

    story.append(Paragraph("Test 1: ECM Timestep Convergence", styles["Heading2"]))
    story.append(
        _case_table(
            [
                ("Status", "PASS" if timestep["passed"] else "FAIL"),
                ("Runs", "dt = 1.0, 0.5, 0.25 s"),
                ("Case", "/workspace/cases/lumped_solid clones"),
                ("maxDi", "100"),
    ...
    story.append(
        _case_table(
            [
                ...
                ("Case", "/workspace/cases/lumped_solid clone"),
                ("Current", "0 A"),
