...
def _infer_params_schema(module_path: Path) -> tuple[str, list[str]]:
    try:
        src = module_path.read_text(encoding="utf-8", errors="ignore")
    except Exception:
        return "unknown", []

    lookup_style = ["Q_Ah", "T_degC", "R0_Ohm", "R_Ohm_1", "R_Ohm_2", "C_F_1", "C_F_2"]
    soc_style = ["SOC", "OCV_V", "R0_Ohm", "R1_Ohm", "R2_Ohm", "tau1_s", "tau2_s", "gamma_hyst_V"]

    if all(token in src for token in ("Q_Ah", "T_degC")):
        return "lookup_qah_temp", lookup_style
    if all(token in src for token in ("SOC", "OCV_V", "gamma_hyst_V")):
        return "soc_table", soc_style
    return "unknown", []


def _validate_params_schema(module_path: Path, params_columns: list[str]) -> tuple[str, list[str]]:
    schema_name, required = _infer_params_schema(module_path)
    if schema_name == "unknown":
        return schema_name, required
    missing = [name for name in required if name not in params_columns]
    if missing:
        raise ECMBackendError(
            "params.csv schema is incompatible with the supplied ecm_step.py; "
            f"detected_module_schema={schema_name}; expected_columns={required}; "
            f"missing_columns={missing}; actual_columns={params_columns}; "
...
class EcmStepBackend(BaseECMBackend):
    ...
    def __init__(
        ...
        *,
        ...
        detected_schema, expected_cols = _validate_params_schema(module_path, params_cols)
        ...
        try:
            ...
            raise ECMBackendError(
                ...
                f"detected_module_schema={detected_schema}; expected_columns={expected_cols}; "
