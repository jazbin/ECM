...
ECM sub-iterations (optional, partitioned subcycling):
```
subIterations      1;        // >1 enables ECM substeps within one CFD step
subIterationResult average;  // average|last
```
When `subIterations > 1`, `ecmCoupler` splits the CFD `deltaT` into equal
substeps, calls the ECM multiple times, and either averages the returned heat
(`average`) or keeps the final substep (`last`). This does not re-solve the
thermal field inside the CFD step; it is a partitioned ECM subcycling option.

Temporal interpolation (optional, between CFD steps):
```
temporalInterpolation hold;   // hold|linear (default hold)
```
When set to `linear`, the applied heat is averaged between the previous and
current ECM outputs (trapezoidal in time) to reduce step artifacts.
...
- Subcycling interpolation: set `ECM_INTERP_MODE=linear` (default) to linearly
  extrapolate `qVol` between ECM fires when `ECM_CALL_EVERY_N_STEPS>1`, or set
  `ECM_INTERP_MODE=hold` to preserve the stepwise hold behavior.
