...
def _infer_module_tokens(module_path: Path) -> list[str]:
    try:
    ...
    tokens: list[str] = []
    for token in ("Q_Ah", "T_degC", "R_Ohm_1", "R_Ohm_2", "C_F_1", "C_F_2"):
        if token in src:
            tokens.append(token)
    return tokens
...
def _validate_external_schema(
    ...
    module_tokens = _infer_module_tokens(module_path)
    ...
    if not params_are_external and not params_are_soc_legacy:
        ...
        raise ECMBackendError(
            "This package supports only the external lookup-table ECM schema. "
            f"params.csv is missing required columns {missing_params}; "
            f"expected_columns={EXTERNAL_PARAMS_COLUMNS}; actual_columns={params_columns}; "
            f"module={module_path}; detected_module_tokens={module_tokens}"
        )
    ...
    if missing_cellprops:
        raise ECMBackendError(
            "This package supports only the external lookup-table ECM schema. "
            f"cellprops.csv is missing required columns {missing_cellprops}; "
            f"expected_columns={EXTERNAL_CELLPROPS_COLUMNS}; actual_columns={cellprops_columns}; "
            f"module={module_path}; detected_module_tokens={module_tokens}"
        )

    return EXTERNAL_PARAMS_COLUMNS, module_tokens
...
def _add_external_alias_columns(params_df):
    df = params_df.copy()
...
class EcmStepBackend(BaseECMBackend):
    ...
    def __init__(
        ...
        *,
        ...
        expected_cols, module_tokens = _validate_external_schema(
            module_path,
            params_cols,
            cellprops_cols,
        )
        ...
        if not _has_columns(params_cols, EXTERNAL_PARAMS_COLUMNS):
            ...
            params_df = _add_external_alias_columns(params_df)
            params_cols = [str(c) for c in params_df.columns]
        ...
        for attempt in range(2):
            try:
                ...
                if missing_name in params_df.columns:
                    break
                synthesized = False
                alias_candidates = {
                    "E_OCV_dch_V": ("OCV_V",),
                ...
                if missing_name in alias_candidates:
                    for source in alias_candidates[missing_name]:
                        if source in params_df.columns:
                            params_df[missing_name] = params_df[source]
                            synthesized = True
            ...
            raise ECMBackendError(
                ...
                f"detected_module_tokens={module_tokens}; "
