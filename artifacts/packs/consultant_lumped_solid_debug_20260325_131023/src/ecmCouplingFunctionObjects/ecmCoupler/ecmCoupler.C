...
:
    ...
    command_(dict.lookupOrDefault<string>("command", "python3 ecm/ecm_coupler.py")),
    ...
    lastGoodQByKey_()
{
    ...
    if (readLastGoodSnapshot() && (Pstream::master() || !Pstream::parRun()))
    {
...
bool ecmCoupler::writeInput(const labelList& keys, const scalarField& Tvals, const uint64_t stepId) const
{
    ...
    h.time = mesh_.time().value();
    h.deltaT = mesh_.time().deltaTValue();
...
scalar ecmCoupler::updateFieldsElementWise(const labelList& keys, const HashTable<scalar, label>& qByKey)
{
    ...
    forAll(cells, i)
    {
        ...
        HashTable<scalar, label>::const_iterator iter = qByKey.find(key);
        if (!iter.found())
        {
            nMissing++;
...
) const
{
    ...
    const bool resetState = wrapperStateReset_ || (stepId <= 1) || (lastGoodStepId_ > 0 && stepId <= lastGoodStepId_);
...
bool ecmCoupler::execute()
{
    ...
    if (ioMode_ == "jsonWrapper")
    {
        ...
        currentA_ = currentVal;

        if (!writeWrapperJsonInput(TcellC, mesh_.time().deltaTValue(), mesh_.time().value(), currentA_, stepId_))
        {
            outputOk = false;
        }

        if (outputOk && !runExternal())
        {
            outputOk = false;
        }
        ...
        scalar qOut = 0.0;
        scalar vT = 0.0;
        if (outputOk && !readWrapperJsonOutput(qOut, vT))
        {
            outputOk = false;
        }
        ...
        if (outputOk)
        {
            qByKey.insert(lumpedKey, qOut);
            outStepId = stepId_;
        }
    }
    else if (ioMode_ == "cliWrapper")
    {
        if (!lumpedMode)
        {
            FatalErrorInFunction
        ...
        const string cmd = buildWrapperCommand(TcellC, mesh_.time().deltaTValue());
        command_ = cmd;
        ...
        scalar qOut = 0.0;
        scalar vT = 0.0;
        if (outputOk && !readWrapperCsv(qOut, vT))
        {
            outputOk = false;
        }
    ...
    {
        if (!writeInput(keys, Tsend, stepId_))
        {
            outputOk = false;
        }
        ...
        if (outputOk && !readOutput(qByKey, stepId_, outStepId))
        {
            outputOk = false;
        }
    }
    ...
    if (adaptiveRelaxation_)
    {
        ...
        if (lumpedMode)
        {
            ...
            if (lumpedOutput_ == "totalPower")
            {
                qVol = (totalVol > SMALL) ? qOut / totalVol : 0.0;
            }
            qSumEstimate = qVol * totalVol;
        }
        else
        {
            const scalarField& V = mesh_.V();
            forAll(cells, i)
            {
                HashTable<scalar, label>::const_iterator iter = qByKey.find(keysLocal[i]);
                if (iter.found())
                {
                    qSumEstimate += (*iter) * V[cells[i]];
                }
            }
            reduce(qSumEstimate, sumOp<scalar>());
        }
    ...
    if (lumpedMode)
    {
        qSum = updateFieldsLumped(cells, lumpedKey, qByKey);
    }
    else
    {
        qSum = updateFieldsElementWise(keysLocal, qByKey);
    }
    ...
    if (!writeLastGoodSnapshot(qByKey, outStepId, lastGoodTime_, qSum, lastReturnCode_))
    {
        if (Pstream::master() || !Pstream::parRun())
        {
