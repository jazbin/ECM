...
SUMMARY_DIAGNOSTIC_FIELDS = [
    ...
    "Q_AH",
    "dt_since_ecm_s",
    "wrapper_state_reinitialized",
    "wrapper_state_reinitialized_reason",
]
...
def _summary_csv_path(args: argparse.Namespace | None) -> Path | None:
    if args is None:
        return None
    explicit = getattr(args, "summary_csv", None)
    if explicit:
    ...
    try:
        ...
        return None
    return state_path.parent / "ecm_wrapper_summary.csv"
...
def _append_summary_csv(
    ...
    path = _summary_csv_path(args)
    if path is None:
        return

    fieldnames = SUMMARY_BASE_FIELDS + SUMMARY_DIAGNOSTIC_FIELDS
    row: dict[str, Any] = {
        ...
        "T_feedback_degC": float(request.t_jellyroll_degc),
        "Q_GEN_W": float(q_gen_w),
        "V_T_V": "" if v_t_v is None else float(v_t_v),
        "q_ah": float(state_next.q_ah),
        "v_rc1_v": float(state_next.v_rc[0]),
        "v_rc2_v": float(state_next.v_rc[1]),
        "hysteresis": float(state_next.hysteresis),
        "ecm_interpolated": bool(diagnostics.get("ecm_interpolated", False)),
    }
    for key in SUMMARY_DIAGNOSTIC_FIELDS:
    ...
    try:
        ...
        _log_diagnostic(args, f"summary csv append failed path={path}: {exc}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional path for a clean per-step ECM summary CSV. "
            "Default: <state_file_dir>/ecm_wrapper_summary.csv"
        ),
    )
