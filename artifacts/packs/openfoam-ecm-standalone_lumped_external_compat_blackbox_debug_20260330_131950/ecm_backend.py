...
EXTERNAL_PARAMS_COLUMNS = ["Q_Ah", "T_degC", "R0_Ohm", "R_Ohm_1", "R_Ohm_2", "C_F_1", "C_F_2"]
EXTERNAL_CELLPROPS_COLUMNS = ["capacity_Ah", "T_ref_degC"]
SOC_PARAMS_COLUMNS = ["SOC", "OCV_V", "R0_Ohm", "R1_Ohm", "R2_Ohm", "tau1_s", "tau2_s", "gamma_hyst_V"]


def _validate_supported_inputs(
    params_columns: list[str],
    cellprops_columns: list[str],
) -> list[str]:
    params_are_external = _has_columns(params_columns, EXTERNAL_PARAMS_COLUMNS)
    params_are_soc_legacy = _has_columns(params_columns, SOC_PARAMS_COLUMNS)

    if not params_are_external and not params_are_soc_legacy:
        missing_params = [name for name in EXTERNAL_PARAMS_COLUMNS if name not in params_columns]
        raise ECMBackendError(
            "Unsupported params.csv schema. "
            f"Accepted inputs are either external lookup-table columns {EXTERNAL_PARAMS_COLUMNS} "
            f"or legacy SOC-table columns {SOC_PARAMS_COLUMNS}; "
            f"actual_columns={params_columns}; missing_external_columns={missing_params}"
        )

    missing_cellprops = [name for name in EXTERNAL_CELLPROPS_COLUMNS if name not in cellprops_columns]
    if missing_cellprops:
        raise ECMBackendError(
            "cellprops.csv is missing required columns for compatibility mode. "
            f"expected_columns={EXTERNAL_CELLPROPS_COLUMNS}; "
            f"actual_columns={cellprops_columns}; missing_columns={missing_cellprops}"
        )

    return EXTERNAL_PARAMS_COLUMNS


def _has_columns(columns: list[str], required: list[str]) -> bool:
    return all(name in columns for name in required)


def _adapt_soc_table_to_external_lookup(params_df, cellprops_df):
    import pandas as pd

    params_columns = [str(c) for c in params_df.columns]
    if not _has_columns(params_columns, SOC_PARAMS_COLUMNS):
        raise ECMBackendError(
            "params.csv is neither an external lookup table nor an adaptable SOC table; "
            f"actual_columns={params_columns}"
        )
    ...
    try:
        ...
        raise ECMBackendError(f"Failed to read capacity_Ah/T_ref_degC from cellprops.csv: {exc}") from exc

    if not math.isfinite(capacity_ah) or capacity_ah <= 0.0:
        raise ECMBackendError(f"Invalid capacity_Ah in cellprops.csv: {capacity_ah}")
    if not math.isfinite(t_ref_degc):
        raise ECMBackendError(f"Invalid T_ref_degC in cellprops.csv: {t_ref_degc}")

    # Build a small 2D temperature grid from the 1D SOC table so external
    # lookup-table ECM implementations can interpolate in (Q_Ah, T_degC).
    temp_offsets = (-10.0, 0.0, 10.0)
    rows: list[dict[str, float]] = []

    for _, src in params_df.iterrows():
        soc = float(src["SOC"])
        soc = _clamp(soc, 0.0, 1.0)
        q_ah = capacity_ah * (1.0 - soc)

        ocv_ref = float(src["OCV_V"])
        r0_ref = max(float(src["R0_Ohm"]), 1.0e-9)
        r1_ref = max(float(src["R1_Ohm"]), 1.0e-9)
        r2_ref = max(float(src["R2_Ohm"]), 1.0e-9)
        tau1_s = max(float(src["tau1_s"]), 1.0e-9)
        tau2_s = max(float(src["tau2_s"]), 1.0e-9)
        gamma_ref = max(float(src["gamma_hyst_V"]), 0.0)

        for offset in temp_offsets:
            t_degc = t_ref_degc + offset
            # Reuse the mock backend's mild temperature trends so the synthetic
            # lookup table stays well-conditioned for 2D interpolation.
            r0 = r0_ref * math.exp(-0.018 * offset)
            r1 = r1_ref * math.exp(-0.010 * offset)
            r2 = r2_ref * math.exp(-0.006 * offset)
            c1 = tau1_s / max(r1, 1.0e-9)
            c2 = tau2_s / max(r2, 1.0e-9)
            ocv = ocv_ref + 0.0006 * offset
            rows.append(
                {
                    "SOC": float(soc),
                    "Q_Ah": float(q_ah),
                    ...
                    "C_F_1": float(c1),
                    "C_F_2": float(c2),
                    "gamma_hyst_V": float(gamma_ref),
                }
            )

    adapted = pd.DataFrame(rows)
    adapted.sort_values(["Q_Ah", "T_degC"], inplace=True, kind="mergesort")
    adapted.reset_index(drop=True, inplace=True)
    return adapted
...
def _capacity_from_cellprops(cellprops_df) -> float | None:
    try:
        value = float(cellprops_df["capacity_Ah"].iloc[0])
    except Exception:
        return None
    return value if math.isfinite(value) else None


def _tref_from_cellprops(cellprops_df) -> float | None:
    try:
        value = float(cellprops_df["T_ref_degC"].iloc[0])
    except Exception:
        return None
    return value if math.isfinite(value) else None


def _try_add_column_from_rule(params_df, target: str, source: str):
    if target in params_df.columns or source not in params_df.columns:
        return params_df, False
    df = params_df.copy()
    df[target] = df[source]
    return df, True


def _try_add_derived_column(params_df, target: str):
    df = params_df.copy()
    if target in df.columns:
        return df, False
    normalized = _normalized_column_name(target)
    if "tau1" in normalized and _has_columns(list(df.columns), ["R_Ohm_1", "C_F_1"]):
        df[target] = df["R_Ohm_1"] * df["C_F_1"]
        return df, True
    if "tau2" in normalized and _has_columns(list(df.columns), ["R_Ohm_2", "C_F_2"]):
        df[target] = df["R_Ohm_2"] * df["C_F_2"]
        return df, True
    return params_df, False


def _try_add_constant_column(params_df, target: str, value: float):
    if target in params_df.columns:
        return params_df, False
    df = params_df.copy()
    df[target] = float(value)
    return df, True


def _try_add_expression_column(params_df, target: str, values):
    if target in params_df.columns:
        return params_df, False
    df = params_df.copy()
    df[target] = values
...
def _synthesize_missing_params_column(params_df, cellprops_df, missing_name: str):
    normalized = _normalized_column_name(missing_name)
    exact_alias_sources: dict[str, str] = {
        "OCV_V": "OCV_V",
        "E_OCV_dch_V": "OCV_V",
        "E_OCV_chg_V": "OCV_V",
        "E_OCV_ch_V": "OCV_V",
        ...
        "C_F1": "C_F_1",
        "C_F2": "C_F_2",
        "gamma_hyst_V": "gamma_hyst_V",
    }

    if missing_name in exact_alias_sources:
        return _try_add_column_from_rule(params_df, missing_name, exact_alias_sources[missing_name])

    if "ocv" in normalized and normalized.endswith("v"):
        return _try_add_column_from_rule(params_df, missing_name, "OCV_V")
    if "r0" in normalized and "ohm" in normalized:
        return _try_add_column_from_rule(params_df, missing_name, "R0_Ohm")
    if ("r1" in normalized or "rohm1" in normalized) and "ohm" in normalized:
        return _try_add_column_from_rule(params_df, missing_name, "R_Ohm_1")
    if ("r2" in normalized or "rohm2" in normalized) and "ohm" in normalized:
        return _try_add_column_from_rule(params_df, missing_name, "R_Ohm_2")
    if ("c1" in normalized or "cf1" in normalized) and normalized.endswith("f"):
        return _try_add_column_from_rule(params_df, missing_name, "C_F_1")
    if ("c2" in normalized or "cf2" in normalized) and normalized.endswith("f"):
        return _try_add_column_from_rule(params_df, missing_name, "C_F_2")
    if "tau1" in normalized:
        return _try_add_derived_column(params_df, missing_name)
    if "tau2" in normalized:
        return _try_add_derived_column(params_df, missing_name)
    if "gamma" in normalized or "hyst" in normalized:
        return _try_add_column_from_rule(params_df, missing_name, "gamma_hyst_V")
    if normalized == "soc" or normalized.endswith("soc") or normalized.startswith("soc"):
        return _try_add_column_from_rule(params_df, missing_name, "SOC")
    if normalized in {"dod", "depthofdischarge"} and "SOC" in params_df.columns:
        return _try_add_expression_column(params_df, missing_name, 1.0 - params_df["SOC"])
    if normalized in {"qah", "qdchah", "qremah", "qdischargedah"} or ("q" in normalized and normalized.endswith("ah")):
        return _try_add_column_from_rule(params_df, missing_name, "Q_Ah")
    if ("t" in normalized and normalized.endswith("degc")) or normalized in {"tcellc", "tcelldegc", "tdegc"}:
        return _try_add_column_from_rule(params_df, missing_name, "T_degC")
    if "tref" in normalized and ("degc" in normalized or normalized.endswith("c")):
        tref_degc = _tref_from_cellprops(cellprops_df)
        if tref_degc is not None:
            return _try_add_constant_column(params_df, missing_name, tref_degc)
    if ("capacity" in normalized or "qmax" in normalized or "qnom" in normalized) and normalized.endswith("ah"):
        capacity_ah = _capacity_from_cellprops(cellprops_df)
        if capacity_ah is not None:
            return _try_add_constant_column(params_df, missing_name, capacity_ah)
    if "eta" in normalized or "eff" in normalized:
        return _try_add_constant_column(params_df, missing_name, 1.0)

    return params_df, False


def _add_external_alias_columns(params_df, cellprops_df):
    df = params_df.copy()
    ...
    capacity_ah = _capacity_from_cellprops(cellprops_df)
    if capacity_ah is not None:
        for name in ("capacity_Ah", "Capacity_Ah", "Q_max_Ah", "Q_nom_Ah"):
            if name not in df.columns:
                df[name] = capacity_ah

    tref_degc = _tref_from_cellprops(cellprops_df)
    if tref_degc is not None:
        for name in ("T_ref_degC", "T_ref_C"):
            if name not in df.columns:
                df[name] = tref_degc

    if "SOC" not in df.columns and "Q_Ah" in df.columns and capacity_ah is not None and capacity_ah > 0.0:
        df["SOC"] = 1.0 - (df["Q_Ah"] / capacity_ah)
        df["SOC"] = df["SOC"].clip(lower=0.0, upper=1.0)
    if "DOD" not in df.columns and "SOC" in df.columns:
        df["DOD"] = 1.0 - df["SOC"]

    if "tau1_s" not in df.columns and _has_columns(list(df.columns), ["R_Ohm_1", "C_F_1"]):
        df["tau1_s"] = df["R_Ohm_1"] * df["C_F_1"]
    if "tau2_s" not in df.columns and _has_columns(list(df.columns), ["R_Ohm_2", "C_F_2"]):
        df["tau2_s"] = df["R_Ohm_2"] * df["C_F_2"]

    if "gamma_hyst_V" not in df.columns:
        df["gamma_hyst_V"] = 0.0
...
class EcmStepBackend(BaseECMBackend):
    ...
    def __init__(
        ...
        *,
        ...
        params_cols = [str(c) for c in params_df.columns]
        cellprops_cols = [str(c) for c in cellprops_df.columns]

        expected_cols = _validate_supported_inputs(
            params_cols,
            cellprops_cols,
        )
        adapted_from = "external_lookup"
        added_aliases: list[str] = []
        if not _has_columns(params_cols, EXTERNAL_PARAMS_COLUMNS):
            params_df = _adapt_soc_table_to_external_lookup(params_df, cellprops_df)
            cols_before_alias = set(str(c) for c in params_df.columns)
            params_df = _add_external_alias_columns(params_df, cellprops_df)
            params_cols = [str(c) for c in params_df.columns]
            added_aliases = sorted(set(params_cols) - cols_before_alias)
            adapted_from = "soc_table"
            missing_after_adapt = [name for name in EXTERNAL_PARAMS_COLUMNS if name not in params_cols]
            if missing_after_adapt:
                raise ECMBackendError(
                    "Failed to adapt SOC-style params.csv to the external lookup-table schema; "
                    f"missing_columns_after_adapt={missing_after_adapt}; actual_columns={params_cols}; "
                    f"module={module_path}"
                )

        else:
            cols_before_alias = set(params_cols)
            params_df = _add_external_alias_columns(params_df, cellprops_df)
            params_cols = [str(c) for c in params_df.columns]
            added_aliases = sorted(set(params_cols) - cols_before_alias)
        ...
        for _attempt in range(32):
            try:
                ...
                last_exc = exc
                last_tb = traceback.format_exc().strip()
                missing_name = str(exc).strip("'\"")
                missing_sequence.append(missing_name)
                if missing_name in params_df.columns or missing_sequence.count(missing_name) > 1:
                    break
                params_df, synthesized = _synthesize_missing_params_column(
                    params_df,
                    cellprops_df,
                    missing_name,
                )
                if not synthesized:
                    break
                if missing_name not in added_aliases:
                    added_aliases.append(missing_name)
                params_cols = [str(c) for c in params_df.columns]
            except Exception as exc:
                last_exc = exc
                last_tb = traceback.format_exc().strip()
            ...
            exc = last_exc if last_exc is not None else ECMBackendError("unknown build_step_cache failure")
            raise ECMBackendError(
                "build_step_cache failed: "
                f"{exc}; module={module_path}; params_path={Path(params_path).resolve()}; "
                f"cellprops_path={Path(cellprops_path).resolve()}; "
                f"package_schema=external_lookup_qah_temp; expected_columns={expected_cols}; "
                f"adapted_from={adapted_from}; "
                f"added_aliases={sorted(added_aliases)}; "
                f"missing_sequence={missing_sequence}; "
                f"params_columns={params_cols}; cellprops_columns={cellprops_cols}; "
                f"traceback={last_tb}"
            ) from exc
        if last_exc is not None and 'lookup_cache' not in locals():
            exc = last_exc
            raise ECMBackendError(
                "build_step_cache failed: "
                f"{exc}; module={module_path}; params_path={Path(params_path).resolve()}; "
