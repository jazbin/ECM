...
**CONDITIONAL.** JellyRoll (Mandrel+Jellyroll, 100% exact) and the top/+Ve hardware (100% exact subdivision of Cap) map exactly today. The Can body requires a 3-way piecewise split (untested in STAR). The bottom/−Ve hardware has no OF-domain target at all and requires an approximation (Option D recommended) rather than an exact reduction. The topological findings above (Can crosses reference domains, bottom hardware has no OF target, confirmed metallic contact-chain shortcut) hold regardless of final dimensions and are the basis for this CONDITIONAL rating — the ~44–84% coverage numbers do not factor into it and must not be cited as a reason for CONDITIONAL vs. some other rating.
