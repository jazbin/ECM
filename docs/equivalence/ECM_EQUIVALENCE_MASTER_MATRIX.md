...
| STAR-012 | S0-A establishes actual STAR Region topology for the Can body: does STAR create sub-Regions from the Can body (by clipping) or one monolithic Can Region? | Test plan S0-A | Region construction known; clipped sub-Regions would simplify material assignment; monolithic Region → STAR-015 needed | Unknown | Not tested | NOT TESTED | Region topology determines whether STAR-015 is a real blocker or resolved by STAR's own sub-Region construction | S0-A (volume report + Region tree for Can body specifically) | S0-A | GEO-020 |
...
| STAR-015 | If S0-A shows monolithic Can Region: STAR can assign distinct thermal material sub-zones within one Region (3-way Can split: JR-equivalent annular inner zone, Can-equivalent annular wall, unresolved bottom) | Gated on STAR-012 result; test plan Test A hard STOP if needed | 3 material zones within one Region; only required if STAR does not auto-create sub-Regions | Unknown; not standard STAR workflow; only triggered if STAR-012 shows monolithic Can | Gated — not testable until STAR-012 resolved | NOT TESTED | If STAR-012 shows monolithic Region AND STAR-015 fails → hard STOP on Test A material mapping; Can geometry must be restructured | S0-A result → if monolithic, escalate to dedicated STAR capability test | Test A / STAR capability test | STAR-012 |
...
| ELEC | 12 | 4 | 4 | 0 | 0 | 4 | 0 |
| DIST | 5 | 2 | 0 | 0 | 0 | 3 | 0 |
| STAR | 16 | 1 | 0 | 0 | 0 | 15 | 0 |
| VAL | 17 | 0 | 1 | 1 | 0 | 15 | 0 |
| RUN | 9 | 5 | 3 | 0 | 0 | 1 | 0 |
| **Total** | **139** | **15** | **20** | **38** | **0** | **66** | **0** |
