...
The single BDS `Can` solid is a *filled envelope* spanning the entire cell length (z_of ≈ −2.21mm to +67.79mm), not a thin wall. Boolean intersection shows 83.7% of its volume intersects the **OpenFOAM JR reference domain** (the spatial zone from the OF mesh after coordinate registration — see Registration note above). This is because the BDS Can solid is a solid bounding shell that the BDS Jellyroll/Mandrel sit *inside* radially, not a hollowed wall — a common BDS/Battery-Design-Studio export convention for the outer housing. **Important distinction:** this 83.7% is an intersection with the OF reference domain, NOT a BDS-body overlap. The BDS Can and BDS Jellyroll have a confirmed radial gap of 0.059504 mm (Can ID = 18.000 mm, JR OD = 17.881 mm) and do NOT volumetrically overlap as BDS bodies. Only 9.7% of Can's volume lands in the OpenFOAM Can domain and 3.5% in Cap; 3.2% falls entirely outside all three reference domains (below OpenFOAM's z=0 origin — see Key finding 2). **A literal "assign Can properties to the whole BDS Can body" mapping would put steel/Can thermal properties (ρ=8000, k=16 isotropic) into 83.7% of what OpenFOAM treats as anisotropic JellyRoll (k_z=29, k_r=1.4) — a severe, easily-missed error.** This body requires piecewise material assignment (see `BDS_TO_OPENFOAM_THERMAL_MAPPING.md`), consistent with the `EXACT PIECEWISE UNION SUPPORTED` finding in `CAN_CAP_REDUCTION_DECISION.md`.
