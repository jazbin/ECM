...
- **S0-C — Three-part thermal capability check.** C.1: Can STAR assign an anisotropic thermal conductivity tensor (independent kr, kθ, kz) in a cylindrical coordinate frame? Required for Jellyroll (MAT-006) and Cap-equivalent (MAT-012). UI inspection only, no material change required. C.2: Can a Region (`+Ve Tab Stem`) retain its +Tab electrical role (STAR-005) while thermal material is changed independently? Each test in its own `.sim` copy. C.3 (new): Can a Core Part Region (Jellyroll or as confirmed by STAR) retain its Core Part assignment (STAR-014) while a non-default thermal conductivity is applied? Each test in its own `.sim` copy.
- **S0-D — Electrical role vs. thermal path suppression.** Four mechanisms (D1–D4) tested on **`−Ve Tab Stem`** (negative/bottom electrical path). Each mechanism is tested in its **own independent `.sim` file** starting from the clean baseline — D1, D2, D3, D4 are not sequential. D1: low-k; D2: Energy exclusion; D3: adiabatic interface; D4: explicit contact resistance (STAR-013). D4 records exact model name, input units, and quantity type.
