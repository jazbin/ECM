#!/usr/bin/env python3
"""
plot_dist_socfix_vs_validation.py
Compare distributed SOC-fix run vs validation data and lumped reference (full 3600s).
3-page PDF:
  Page 1: T overlay + Q overlay
  Page 2: deltaT + deltaQ
  Page 3: running RMSE(T) + running RMSE(Q)
"""
...
LUMP_CSV  = Path("/workspace/artifacts/h_sweep/caseLong_h_sweep_20260501_191029/h_110p0/ecm_wrapper_summary.csv")
...
def running_rmse(model, ref_interp):
    d2 = (model - ref_interp) ** 2
    return np.sqrt(np.cumsum(d2) / np.arange(1, len(d2) + 1))
...
def main():
    ...
    # Interpolate validation onto model time grids for error metrics
    ...
    with pdf_backend.PdfPages(str(OUT_PDF)) as pdf:
        ...
        fig.suptitle(
            "Distributed ECM (SOC-fix) vs Lumped vs Validation — 3600 s",
        ...
        fig.subplots_adjust(hspace=0.42)
        fig.suptitle(
            "Running RMSE vs Validation — Distributed (SOC-fix) vs Lumped",
            fontweight="bold", fontsize=13
        )

        ax = axes[0]
        ax.plot(lump_t, running_rmse(lump_T, lump_T_i), color=C_LUMP, lw=1.8,
                label=f"Lumped h=110  final={rT_lump:.3f} degC")
        ax.plot(dist_t, running_rmse(dist_T, dist_T_i), color=C_DIST, lw=1.5,
                label=f"Distributed (SOC-fix)  final={rT_dist:.3f} degC")
        ax.set_title("Running RMSE — Temperature", fontweight="bold")
        ax.set_xlabel("Time [s]")
        ...
        ax = axes[1]
        ax.plot(lump_t, running_rmse(lump_Q, lump_Q_i), color=C_LUMP, lw=1.8,
                label=f"Lumped h=110  final={rQ_lump:.4f} W")
        ax.plot(dist_t, running_rmse(dist_Q, dist_Q_i), color=C_DIST, lw=1.5,
                label=f"Distributed (SOC-fix)  final={rQ_dist:.4f} W")
        ax.set_title("Running RMSE — Heat Generation", fontweight="bold")
        ax.set_xlabel("Time [s]")
    ...
    print(f"{'Lumped h=110 (full 3600s)':<35} {rT_lump:>10.4f} {bT_lump:>+10.4f} {rQ_lump:>10.4f} {bQ_lump:>+10.4f}")
