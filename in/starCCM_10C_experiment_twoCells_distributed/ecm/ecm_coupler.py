...
def _mapping_signature(mapping_file: str, mapping: dict | None) -> str | None:
    h = hashlib.sha256()
    added = False
    if mapping_file and os.path.exists(mapping_file):
        for path in (
            mapping_file,
            os.environ.get("ECM_CELL_MAP_CSV", "").strip()
            or os.path.join(os.path.dirname(os.path.abspath(mapping_file)), "ecm_cell_map.csv"),
            os.environ.get("ECM_REGION_GEOMETRY_CSV", "").strip()
            or os.path.join(os.path.dirname(os.path.abspath(mapping_file)), "ecm_region_geometry.csv"),
        ):
    ...
    if mapping is None:
        return None
    mesh_keys = sorted(mapping.get("mesh_to_ecm", {}).keys())
    ecm_keys = sorted(mapping.get("ecm_to_mesh", {}).keys())
    payload = json.dumps(
        {
            "synthetic": True,
            "n_mesh": len(mesh_keys),
            "mesh_min": mesh_keys[0] if mesh_keys else None,
            "mesh_max": mesh_keys[-1] if mesh_keys else None,
            "ecm_ids": ecm_keys,
        },
        sort_keys=True,
    ).encode("utf-8")
...
def _maybe_regen_mapping(mapping_path: str) -> None:
    """Regenerate ecm_mapping.csv from ecm_cell_map.csv once per process.

    Runs at most once per process per mapping path (gated by _REGEN_CHECKED).
    This intentionally does not use "mapping is current" heuristics: remeshing can
    preserve row counts and timestamps do not prove cell IDs, centroids, volumes,
    or region labels are still valid.
    ...
    # Stale check — two-tier to avoid reading large CSVs on every step in file-based mode.
    #
    # Tier 1 (fast, every step): read a tiny sentinel file ecm_mapping.csv.ncells that
    # records the confirmed cell count.  Count lines in ecm_cell_map.csv (no CSV parsing).
    # If counts match, the mapping is current — return immediately.
    #
    # Tier 2 (slow, runs once after startup or remesh): count unique meshKeys in the
    # mapping.  Overlap-weighted mappings have more rows than cells (boundary cells appear
    # multiple times), so row count alone is not enough.  On confirmation, write the
    # sentinel so Tier 1 handles all subsequent steps.
    sentinel_path = mapping_path + ".ncells"
    try:
        with open(cell_map_path) as _f:
            n_cells_new = sum(1 for _ in _f) - 1  # exclude header row
    except Exception as _e:
        sys.stderr.write(f"[ecm_coupler] WARN: cannot count cell map rows ({_e}); regenerating mapping.\n")
        n_cells_new = -1

    if n_cells_new > 0 and os.path.exists(mapping_path):
        # Tier 1: sentinel fast-path
        if os.path.exists(sentinel_path):
            try:
                cached_n = int(open(sentinel_path).read().strip())
                if cached_n == n_cells_new:
                    return  # mapping confirmed current this run already
            except Exception:
                pass  # corrupt sentinel — fall through to Tier 2

        # Tier 2: full unique-key scan (runs at most once per mapping lifetime)
        try:
            with open(mapping_path, newline="") as _f:
                _reader = csv.DictReader(_f)
                _key_col = (_reader.fieldnames or ["meshKey"])[0]
                n_existing_keys = len({row[_key_col] for row in _reader})
            if n_existing_keys == n_cells_new:
                # Write sentinel so every subsequent step uses Tier 1
                try:
                    with open(sentinel_path, "w") as _sf:
                        _sf.write(str(n_cells_new))
                except Exception:
                    pass
                sys.stderr.write(
                    f"[ecm_coupler] Mapping current ({n_cells_new} cells) — "
                    f"skipping simplified regen to preserve overlap blending.\n"
                )
                return
            sys.stderr.write(
                f"[ecm_coupler] Mesh changed ({n_existing_keys} → {n_cells_new} cells)"
                f" — regenerating mapping.\n"
            )
        except Exception as _e:
            sys.stderr.write(
                f"[ecm_coupler] WARN: stale-check failed ({_e}); regenerating mapping.\n"
            )

    # Invalidate sentinel before regenerating
    try:
    ...
    sys.stderr.write(
        f"[ecm_coupler] Regenerating mapping from current ecm_cell_map.csv "
        f"({n_regions}×{n_axial}×{n_radial} zones) ...\n"
    )
...
def compute_q_out(h, inputs, keys, temps):
    ...
    current_mapping_signature = _mapping_signature(mapping_file, mapping) if mapping is not None else None
    ...
    if current_mapping_signature:
        previous_mapping_signature = state.get("_mapping_signature")
        if previous_mapping_signature and previous_mapping_signature != current_mapping_signature:
            sys.stderr.write(
                "[ecm_coupler] Mapping signature changed; resetting ECM state/cache so "
                "partition IDs are not reused with a new mesh mapping.\n"
            )
            state = state_defaults()
            reset_flag = True
        elif previous_mapping_signature is None and os.path.exists(state_file):
            sys.stderr.write(
                "[ecm_coupler] Existing state has no mapping signature; resetting once "
                "to bind state to the regenerated mapping.\n"
            )
            state = state_defaults()
            reset_flag = True
        state["_mapping_signature"] = current_mapping_signature
            ...
            if distributed_mode == "partitionstates":
                ...
                if n_regions <= 1:
                    ...
                    combined_diag = {
                        ...
                        "n_regions": n_regions,
                        "current_per_region_A": float(current_per_region),
                        "Q_total_W": 0.0,
                        "regions": [],
                    }
                    sys.stderr.write(
                        f"[ECM-MR] multi-region dispatch: n_regions={n_regions} "
                        f"I_pack={current_a:.3f} A  I_per_region={current_per_region:.3f} A  "
                    ...
                    for r in range(n_regions):
                        ...
                        sys.stderr.write(
                            ...
                            f"I={current_per_region:.3f} A\n"
                        )
                        r_qvol, r_next, r_diag = run_ecm_step_parallel_branches(
                            ecm_ids=r_ids,
                            ecm_temps_k=r_temps,
                            dt_s=dt_for_ecm,
                            current_a=current_per_region,
