...
| `H004-5`: Detailed Builder `m_dSepFeedLength_mm = 0` and/or `m_dSepTailLength_mm = 0` feeds a mandatory zero extrusion | both remain zero; STAR-install cylindrical refs use nonzero values such as 10/85; exact error is zero extrusion | **not yet runtime-tested with both made nonzero** | pending | **`LEADING_HYPOTHESIS / UNPROVEN`** |

Important: `H004-5` must **not** be described as the confirmed cause until a controlled runtime candidate progresses past `E004`.
...
LEADING_UNTESTED_HYPOTHESIS = Detailed Builder separator feed/tail zero lengths
```
